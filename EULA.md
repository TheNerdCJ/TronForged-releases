...
3.2 No cloud, nothing received, nothing reachable. The Licensor operates no cloud, hosted service, account, relay or rendezvous. Every Shop hosts its own server with its own keys; the Licensor holds no key any Shop server trusts and no account on any Shop server, and consoles talk only to that server plus the contacts in Sections 6 and 11.4 (Section 16.4 lists what the Shop SERVER contacts, which is a different and longer list - a console never contacts an AI provider, Section 15.1). Nothing in the Software sends telemetry, crash reports or customer data to the Licensor (redacted records go only to the Shop's server, Section 12.2), and the Licensor cannot access, monitor, update, disable or connect to any Shop's server or computers.
...
5.3 The Patch stage installs PowerShell Gallery modules and runs winget with package agreements accepted on the Shop's behalf, including for the computer's OEM update utility; running it accepts those terms. The Software may also install two Microsoft components on the computer when they are missing, without asking, in the background and at most once a day: the .NET Desktop Runtime, and PowerShell 7 for all users of the computer (into the Program Files folder). Each is downloaded from Microsoft, checked against the checksum Microsoft publishes with it, and installed only if every program file it contains carries a valid Authenticode signature naming Microsoft; anything that fails is discarded and the Software continues using the PowerShell and runtime already present in Windows. Both remain subject to Microsoft's own terms. A technician may install, update or remove PowerShell 7 from the Software's settings; removal affects only a copy the Software installed. The Software installs no kernel driver.
...
10.2 Arming is not optional on a Customer console: a connected Customer computer is armed for unattended access, with no checkbox to switch it off while connected. Connecting also installs a boot-start Windows service ("TronForged Remote Agent"), hosting the agent whenever the console is not running, and the SYSTEM helper service ("TronForgedHelper"), which reaches elevation prompts and the lock and sign-in screens, plus firewall rules named "TronForged Remote Agent". Installing that helper - and the fifteen-minute check that keeps it running - also turns on one Windows security policy, "Disable or enable software Secure Attention Sequence" (SoftwareSASGeneration), set to permit services, which is what lets a technician send Ctrl+Alt+Delete to the computer; the policy value that was there before is recorded, nothing already permitted is removed, and removing the helper puts it back. Arming repeats on every launch and about every fifteen minutes, and the self-repair loop re-installs the helper if it is missing. The only ways out are disconnecting and uninstalling.

10.3 What a technician can do. In a session a technician can see and control the screen, keyboard and mouse - including elevation prompts and the lock and sign-in screens where the helper is present - choose which of the computer's screens to view, send Ctrl+Alt+Delete and other key combinations from a toolbar, and run repair stages, fixes, Downloaded Tools and reports. The remote command surface is a closed list of named capabilities; there is no free-form command execution. The session is encrypted end to end between the two consoles; the Shop server authenticates it and relays ciphertext it cannot read.
...
11.1 It changes things. The engine's cleaning, debloating, disinfecting, resetting, software removal, updates and restarts are changes, some irreversible, and its imaging, restore and data-recovery utilities can overwrite disks. It creates a restore point and a registry backup first, can resume after a restart, and gates the most dangerous operations (a whole-machine restore or a disk clone needs a dry-run plan and a typed confirmation, is limited to accounts the Shop has granted the restore permission, erases the destination disk, and refuses the running Windows disk as a destination - including when it cannot identify that disk), but the decision to run them is yours. The Shop must hold the Customer's authority for the work, take its own backups, and bears the consequences.

11.2 Sensitive material. Optionally, the engine copies the computer's BitLocker recovery keys to its logs folder, recovers product keys and, only on the technician's switch, exports saved Wi-Fi keys in plain text. A forensics collector, when a technician runs it, copies and hashes Windows artifacts - registry hives, browser databases, recycle-bin records and others - into a chain-of-custody bundle without modifying the computer, and on the technician's switch creates a shadow copy. An image of a BitLocker-protected volume holds that volume's contents unencrypted (Section 13.1). Reports are filed on the computer. When a technician has chosen the ticket the work is for, the service report from a run attaches automatically to that ticket on the Shop server, where the Customer can see it, and the run's receipt, quality-control checklists, data-recovery reports (which name the disks, images and folders worked on) and technician-approved customer summaries attach to it as internal files the Customer cannot see. The Shop must treat all of this as confidential Customer material and use it only for the repair.
...
13.4 Restores and clones run from inside Windows only, onto a second disk; a clone captures the running computer and restores that image onto the chosen disk in one job. "Boot in VM" runs an image under Hyper-V for inspection only; Windows may report itself unactivated there, and it is not a way to run a Customer's Windows license. The Shop must test the backups it relies on.
...
15.1 AI features are opt-in and default off. Each feature also has its own switch. Turning AI on switches on the three features that existed before release 0.426.0 (the fleet health summary, the draft customer summary and the server-sizing snapshot of Section 15.2); every feature added since, starting with the ticket brief, stays off until the Shop switches it on by itself. A Shop that turns them on supplies an Anthropic API key or a Claude Code sign-in on the Shop server; consoles never call a model. All AI traffic leaves from the Shop server, to Anthropic, under the Shop's own account and Anthropic's terms; the Licensor is not in the chain.

15.2 What is sent, feature by feature, each only while its own switch is on: a fleet health summary from the redacted diagnostics of Section 12.2; a draft customer summary from the recorded results of one repair run, without the Customer's name, the ticket number or where the report is kept (used only if a technician approves it); a numeric server-sizing snapshot; from release 0.426.0, a brief of ONE support ticket for the technician working it, which carries that ticket's subject, the Customer's own messages on it, and the Shop's replies and internal notes on it; and, from release 0.428.0, a draft of a reply or a resolution for ONE support ticket, which carries that ticket's subject and only the messages the Customer can already see on it - never the Shop's internal notes - and, for a resolution, the summary of work the Software composed from the Shop's work records. A draft is returned to a person at the Shop, who edits and sends it; when a message began as such a draft, the Software adds a line at its end telling the Customer it was written with the help of an AI assistant. The Software cannot label text a person types or pastes in from elsewhere. Text shaped like a password, key or recovery code is masked before a ticket brief is sent; that is a check on the SHAPE of the text, not a guarantee that no secret is sent. Calls are audited by token count and by ticket number, never by message content, and each signed-in person is capped at a fixed number of AI requests a day. The optional training export is an explicit administrator action; the training pipeline itself is not in any release and never runs on a Customer's computer.

15.3 A model never acts on a machine: no tools are declared, the command-line route runs with tools disabled, and model output is never parsed or dispatched, only shown, labelled AI-generated. A draft reaches a Customer only when a person at the Shop sends it. The Shop is responsible for what it sends to its provider, including any personal data in it, and for the provider's terms and cost.
...
17.2 The console lets you request support, run curated safe fixes, read reports, back up files where offered, see your tickets, read what your Shop has written to you about that computer and write back to it, and lets your Shop connect; it is not antivirus, nor a substitute for your own copies of important files. Reading and answering your Shop this way needs no account: it uses the computer's own connection to the Shop server, shows the tickets that belong to that computer, and is limited in message length, in how many messages the computer may send in an hour, and in how much of a long conversation it displays. It does not show you the Shop's internal notes or any file attached to a ticket. If your Shop uses either AI feature of Section 15.2 that reads these conversations, that window tells you so and Section 19.1 says what it means. A technician may also sign in on your computer with their own account (Section 10.7).
...
19.1 To your Shop. The console sends your Shop the reporting in Section 12.2, technician reports, anything you yourself type and send - what you write in a support request and any reply you send to a ticket from that computer, which your Shop's server keeps as part of that ticket - and, if you use it, your backups - over an encrypted connection, and your Shop's server encrypts your backups when it stores them (Section 13.1); no setting turns the reporting off short of disconnecting or uninstalling, and nothing goes to the Licensor. Your Shop can send what you write onward for two purposes, each only if it switches that feature on (Section 15.2): so that a technician can be given a summary of the conversation before working on your computer, and to help write its replies to you. For either, what you write in a support request or a ticket reply is sent from your Shop's server to that Shop's AI provider. While either feature is on, the windows where you read and answer your Shop say so, and a message your Shop sent from a draft the Software's AI feature wrote says so at its end; while both are off, nothing you write leaves your Shop's server. Your Shop chooses this, can switch it off, and is responsible for it and for its provider's terms (Section 15.3). Nothing else you send goes anywhere but your Shop.
...
21.1 Unverified on real hardware. At the effective date the features never exercised on real hardware include unattended access before sign-in and while the console is not running (including its notice), remote update installs and restarts, the TLS front door, dynamic DNS, the move from PostgreSQL to the embedded database, desktop-duplication capture and multiple monitors, whole-machine imaging - capturing an image, restoring or cloning one to a disk, which erases that disk, capturing and re-registering the Windows recovery partition, restoring onto a smaller disk, the refusal of a target disk the Software cannot identify as other than the running Windows disk, and BitLocker detection - and Square charges. Only attended remote desktop has been exercised on real hardware; the README's status section controls.
...
24.3 WITHOUT LIMITING 24.1 AND 24.2, THE LICENSOR IS NOT LIABLE FOR ANY ACT OR OMISSION OF A SHOP, A CUSTOMER OR ANY THIRD PARTY; FOR ANY REMOTE ACCESS, REPAIR, RESTART, WIPE, IMAGING, CLONE OR RESTORE PERFORMED WITH THE SOFTWARE, INCLUDING WITHOUT AUTHORITY OR CONSENT; FOR ANY DATA A SHOP HOLDS, LOSES, DISCLOSES OR FAILS TO DELETE; FOR THE SECURITY OF A SHOP'S SERVER OR NETWORK; OR FOR ANY UNVERIFIED OR UNBUILT FEATURE.
