...
11.1 It changes things. The engine's cleaning, debloating, disinfecting, resetting, software removal, updates and restarts are changes, some irreversible, and its imaging, restore and data-recovery utilities can overwrite disks. It creates a restore point and a registry backup first, can resume after a restart, and gates the most dangerous operations (a whole-machine restore or a disk clone needs a dry-run plan and a typed confirmation, erases the destination disk, and refuses the running Windows disk as a destination), but the decision to run them is yours. The Shop must hold the Customer's authority for the work, take its own backups, and bears the consequences.
