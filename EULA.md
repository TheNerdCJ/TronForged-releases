...
9.2 The Shop must obtain the Customer's informed agreement to this Agreement and to unattended remote access, collected on screen at enrolment or when the console is later connected. That text says that by connecting the Customer allows the Shop to reach the PC to help, including when the Customer is away, and to install Windows updates and restart it; that a notice appears whenever a technician is connected and before any restart; that the console starts with Windows; and that it can be removed any time from the tray. The recorded consent moves with a reinstall; a recorded "No" stays "No". The Shop must not click that consent for a Customer without instruction, enrol a computer whose owner declined, re-enrol a disconnected computer without fresh consent, or ignore a revocation (Section 14).

9.3 The Shop must understand Section 10 and be able to explain it to a Customer. The consent text does not mention the sign-in screen, the boot-start service, the SYSTEM helper or the absence of live notice while the console is not running (Sections 10.2 and 10.4); the Shop must tell the Customer these things before connecting. The Software's banner, prompts, notices and logs are tools for notice; whether they are sufficient where the Shop operates is the Shop's judgment.
